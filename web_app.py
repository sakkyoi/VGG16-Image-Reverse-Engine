...
def query(image, mode, noise, noise_seed, mean, var, amount, salt_vs_pepper):
    if image is None or noise_seed is None:
    ...
    # noise == 'none' pass
    if noise == 'gaussian' or noise == 'speckle':
    ...
    elif noise == 'localvar' or noise == 'poisson':
        ...
        query_img = random_noise(query_img / 255, mode=noise, rng=int(noise_seed), amount=amount,
                                 salt_vs_pepper=salt_vs_pepper, clip=True)
        query_img = np.array(query_img * 255, dtype=np.uint8)

    start = time.time()
    results, scores, length, cluster = None, None, None, None
    if mode == 'normal':
    ...
    return query_img, f'Query time: {query_time} ms', \
        [(result, f'Score: {score}, file: {result}')for result, score in zip(results, scores)], \
        length, f'{cluster}' if mode == 'kmeans' else 'None'
    ...
    iface = gr.Interface(
        ...
        inputs=[
            ...
            gr.Radio(['normal', 'kmeans'], value='normal',
                     info='normal: search all images, kmeans: search images in the same cluster'),
            gr.Radio(['none', 'gaussian', 'localvar', 'poisson', 'salt', 'pepper', 's&p', 'speckle'],
                     value='none', info='noise type (default: none)'),
            gr.Number(label='random_seed', value=0, info='random seed for noise (default: 0)'),
            gr.Slider(label='mean', maximum=1, minimum=0, value=0,
                      info='mean of the noise, works with gaussian and localvar (default: 0)'),
            gr.Slider(label='var', maximum=1, minimum=0, value=0.01,
                      info='variance of the noise, works with gaussian and localvar (default: 0.01)'),
            gr.Slider(label='amount', maximum=1, minimum=0, value=0.05,
                      info='proportion of image pixels to replace with \
                      noise, works with salt, pepper, and s&p (default: 0.05)'),
            gr.Slider(label='salt_vs_pepper', maximum=1, minimum=0, value=0.5,
                      info='proportion of s&p noise (default: 0.5)'),
        ],
        outputs=[
